...
// Loads the last processed document from the backend and allows editing extracted fields
...
export default function Editor() {
  ...
  // Always work with the last document in the list
  const lastDoc       = data?.[data.length - 1];
  const extractedData = lastDoc?.extracted_data ?? {};

  // Local editable copy of the extracted fields
  const [fields, setFields] = useState<Record<string, any>>({});

  // Sync local fields whenever API data arrives or the document changes
  useEffect(() => {
    if (extractedData) setFields(extractedData);
  }, [data]);

  // Update a single field value without touching the others
  const handleChange = (key: string, value: string) =>
    setFields((prev) => ({ ...prev, [key]: value }));

  // JSON preview object — rebuilt on every keystroke for real-time feedback
  ...
  // Converts a snake_case key into a readable label (e.g. "invoice_date" → "INVOICE DATE")
    ...
    return (
      <Box key={key} mb={2.5}>
        <Typography variant="caption" sx={{
          color: colors.blueMuted, letterSpacing: 1.5, fontWeight: 600, fontSize: 11,
        }}>
          {label}
        </Typography>
        <TextField
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 } }}>
      ...
      {/* Main editor layout — shown only when a document is available */}
      {!isLoading && !isError && lastDoc && (
        <Box sx={{ display: "flex", gap: 3, flexDirection: { xs: "column", md: "row" } }}>
          ...
          <Paper sx={{ flex: 1, borderRadius: 3, border: `1px solid ${colors.borderCard}`, p: 3 }}>
            <Typography variant="h6" color={colors.textWhite} fontWeight="bold">
              Extracted Fields
            </Typography>
          ...
          <Paper sx={{
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
              ...
              {/* JSON badge */}
              <Typography variant="caption" sx={{
                color: colors.blueMuted, bgcolor: "#1e3a8a33",
...
// renderJsonSyntax — recursively renders a JSON object with syntax highlighting
// Each value type gets its own color: strings=green, numbers=orange, booleans=pink, null=gray
