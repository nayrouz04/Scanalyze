...
import {
  ...
  Pagination, InputAdornment, Switch, Avatar, CircularProgress, Alert,
} from "@mui/material";
    ...
    <Box sx={{ p: { xs: 2, md: 4 } }}>
      ...
      {loadingUsers ? (
        ...
        <Alert severity="error">
          Impossible de charger les utilisateurs. Vérifiez la connexion à l'API.
        </Alert>
      ) : (
        <Box
          sx={{
            bgcolor: colors.bgCard, borderRadius: 2,
            border: `1px solid ${colors.borderCard}`, p: 2,
          }}
        >
          {/* ── Filtres ── */}
                  ...
                  >
                    ...
                    <TableCell sx={tableCellSx}>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                        ...
                        <Box>
                          <Typography color={colors.textWhite} fontSize={13} fontWeight={500}>
                            {user.full_name}
                          </Typography>
                          <Typography color={colors.textMuted} fontSize={11}>
                            {user.email}
                          </Typography>
                        </Box>
                      </Box>
          ...
          <Box
            sx={{
              ...
              alignItems: "center", mt: 2, flexWrap: "wrap", gap: 1,
            }}
          >
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)}>
        ...
        <DialogContent>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
            <TextField
              label="Full Name" fullWidth value={form.full_name}
            ...
            />
            <TextField
              label="Office Address" fullWidth value={form.office_address}
              onChange={(e) => setForm({ ...form, office_address: e.target.value })}
              sx={inputSx}
            />
            <TextField
              label="Phone (optionnel)" fullWidth value={form.phone_nbr}
              onChange={(e) => setForm({ ...form, phone_nbr: e.target.value })}
              sx={inputSx}
            />
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setCreateOpen(false)} sx={{ color: colors.textMuted }}>
            Cancel
          </Button>
          <Button
            variant="contained"
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)}>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setEditOpen(false)} sx={{ color: colors.textMuted }}>
            Cancel
          </Button>
          <Button
            variant="contained"
            onClick={handleEdit}
            disabled={updating || !form.full_name || !form.email}
            sx={btnPrimarySx}
          >
      ...
      <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)}>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setDeleteOpen(false)} sx={{ color: colors.textMuted }}>
            Cancel
          </Button>
          <Button
            variant="contained"
